...
0. **【最高优先级】页数限制：必须严格控制在 1-2 页 A4 以内**。通过压缩行间距（1.2-1.4）、减小板块间距、缩小字号（正文9-10pt仍可读）、精简描述文字来实现。不得删减任何条目，但可以在不改变原意的前提下缩写过长描述。如果项目经历较多，优先压缩较旧或不太相关的项目。

1. **必须是完整的独立 HTML 文件**，包含 <html><head><body>，所有 CSS 内嵌在 <style> 标签中。
2. **不要引入外部资源**（不要 Google Fonts、CDN 等），使用系统字体栈。
3. **中文排版**：正文使用系统中文字体（如 "PingFang SC", "Microsoft YaHei", "SimSun"），字号恰当（正文9-11pt，标题13-16pt，小标题11-12pt）。
4. **视觉层次**：用小标题、紧凑留白、细线来区分板块，不要用大面积色块。避免无意义的装饰性空白。
...
8. **紧凑排版优先**：margin/padding 尽量小（板块间距 ≤ 8pt，段落间距 ≤ 4pt），让内容占满页面而非大量留白。
